...
from tests.website.ui_tests.conftest import id_company, swagger_url
...
def test_create_driver():
    with allure.step("Cоздание водителя"):
        ...
        browser.element('[placeholder="Введите фамилию"]').type('Автотестов123')
        browser.element('[placeholder = "Введите имя"]').type('Водитель')
        browser.element('[placeholder="Введите отчество"]').type('Тестович')
        browser.element('[placeholder = "Введите номер телефона"]').type('72348050525')
    ...
    with allure.step("Проверка сохранения водителя"):
        browser.element('[class="table__item"]').should(have.text('Автотестов123'))
        browser.element('//*[contains(@class, "phone") and contains(text(), "+7 234 805 05 25")]').click()
